    ...
    def rgb_callback(self, data):
        ...
        if centers:
            ...
            if self.depth_image is not None:
                ...
                # 使用 PnP 求解相机姿态
                if len(identified_points) == 4:
                    image_points = np.array([point[0] for point in identified_points], dtype=np.float32)
                    success, rvec, tvec = cv2.solvePnP(self.world_points, image_points, self.K, None)
                    ...
                    if success:
                        ...
                        # 计算特征中心在相机坐标系下的位置
                        feature_center_world = np.mean(self.world_points, axis=0)
                        feature_center_camera = np.dot(rotation_matrix, feature_center_world) + tvec.flatten()

                        rospy.loginfo("Rotation matrix: %s", rotation_matrix)
                        rospy.loginfo("Translation vector: %s", tvec)
                        rospy.loginfo("Depth: %s", Z)
